...
public class Arkanoid extends JFrame implements KeyListener {
	...
	void testCollision(Paddle mPaddle, Ball mBall) {
		...
		//System.out.println((mBall.x+30) + "    bola   " + mPaddle.x);
		
		double calculo = (mBall.x - mPaddle.x);
		
		if (mBall.x < mPaddle.x) {
			mBall.velocityX = -VELOCIDAD_BOLA;
		} else if(calculo < 15.0){
			System.out.println("EN EL CENTRO");
			mBall.velocityX = -VELOCIDAD_BOLA*0.2;
		} else if(calculo < 50.0){
			System.out.println("ESQUINA DERECHA");
			mBall.velocityX = VELOCIDAD_BOLA*1.4;
		...
		System.out.println(mBall.x - mPaddle.x);
