...
public class Arkanoid2 extends JFrame implements KeyListener {
	...
	public static int CANTIDAD_BLOQUES_X = 1; //11
	public static int CANTIDAD_BLOQUES_Y = 1; //4
	...
	class ScoreBoard {
		...
		void increaseScore() {
			...
			if (bricks.size() == 1 && nivel == 0) {
				...
				win = true;
				text = "";
			} else if (bricks.size() == 1 && nivel == 1 && actualizado) {
				nivel = 2;
			}

			if (bricks.size() == 1 && nivel == 2) {
		...
		while (running) {
			...
			if (!scoreboard.gameOver && !scoreboard.win && !paused) {
				...
				if(nivel == 1) {
					...
					actualizado = true;
					CANTIDAD_BLOQUES_X = 1;
					CANTIDAD_BLOQUES_Y = 1;
